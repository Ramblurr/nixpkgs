...
- `qt.platformTheme` no longer supports the `gtk2` option as GTK 2 had reached end-of-life and will be removed in Nixpkgs.
...
- [`services.ocis`](#opt-services.ocis.enable) now provides the complete
  ownCloud Infinite Scale production upgrade path from 5.0.x to 8.1.x, emits
  the next required package as a warning, and provides the `ocisadm`
  administration wrapper. Existing systems retain the 5.0.x package default;
  follow the [oCIS upgrade documentation](#module-services-ocis-upgrading)
  before selecting each intermediate package.
